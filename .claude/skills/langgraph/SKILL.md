...
description: >
  ...
  stateful, multi-actor AI applications. Covers graph construction,
  state management, cycles and branches, persistence with checkpointers,
  human-in-the-loop patterns, and the ReAct agent pattern. Use when
  building LangGraph agents, stateful workflows, or multi-actor systems.
  Triggers: langgraph, langchain agent, stateful agent, agent graph,
  react agent, state graph, checkpointer.
user-invokable: true
argument-hint: "<LangGraph pattern or agent to build>"
metadata:
  source-url: "vibeship-spawner-skills (Apache 2.0)"
---
